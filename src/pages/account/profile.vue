    ...
    <Collapse simple>
      <Panel v-if="Accesslist.company_profile && Companyprofile!== null" name="1">
        <div class="toggle-item">
          ...
          <div
            class="item-desc"
          >Change your company name, language, and time zone,and other Settings that apply to the company.</div>
        </div>
        <div slot="content" v-if="Companyprofile !== null" class="toggle-content">
          ...
          <div class="profileitemlist marignT12">
            ...
            <div
              class="text_right"
            >{{Companyprofile.info.account_used}}/{{Companyprofile.info.account_num}}</div>
          </div>
          <div class="profileitemlist marignT12">
            <div class="text_left">Number of groups used/upper limit:</div>
            <div
              class="text_right"
            >{{Companyprofile.info.group_used}}/{{Companyprofile.info.group_num}}</div>
          </div>
          <div class="line"></div>
          <div class="profileitemlist">
      ...
      <Panel v-if="Accesslist.domain_anagement && CompanyDomain!== null" name="3">
        <div class="toggle-item">
          ...
          <div
            class="item-desc"
          >You can set the domain name LOGO, you can also cancel your domain name.</div>
        </div>
        <div slot="content" class="toggle-content">
          <div class="profileitemlist">
            <div class="text_left" style=" align-self: end;">LOGO:</div>
      ...
      <div class="form-content">
        <Form ref="phone" :model="formItem" :rules="ruleInline" inline label-position="top">
          ...
          <FormItem style="margin-top:40px">
            ...
            >{{sendDisable ? 'place wite '+ daojishi+'s' : 'Send Verification Code'}}</Button>
          </FormItem>
        </Form>
      </div>
    </Modal>
    ...
    >
      <div class="form-content">
        <Form ref="email" :model="formItem" :rules="ruleInline" inline label-position="top">
          <div>
            <FormItem label="Phone Number:" prop="email" style="width:100%">
              <Input
                style="width: 300px"
...
export default {
  ...
  methods: {
    ...
    handleSubmitPhoneCode() {
      ...
      let country_code = this.formItem.country_code;
      if (phone === "") {
      ...
      if (val === "phone") {
        this.$refs[val].validate(valid => {
          if (valid) {
            ...
            this.$request
              .post("/company/phoneChange", { phone, country_code, code })
              .then(res => {
                if (res.status) {
                  this.$Message.success("success");
                } else {
